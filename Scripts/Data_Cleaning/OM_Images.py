# This script takes the original OM image and splits it into smaller chunks for analysis in the neural network
# Original images *around* 13800x9800 px
...
# SPLITTER
def om_image_split(file_name, save_folder_path, px_square_size):
    ...
    images_vertical = math.floor(len(image[0]) / px_square_size)
    images_horizontal = math.floor(len(image) / px_square_size)

    print(images_vertical)
    print(images_horizontal)
            ...
            for row in range(px_square_size * (image_number_horizontal - 1), px_square_size * image_number_horizontal):
                for column in range(px_square_size * (image_number_vertical - 1),
                    ...
                    split_image_arr[row - (px_square_size * (image_number_horizontal - 1))][
                        column - (px_square_size * (image_number_vertical - 1))] = image[row][column][0]
...
if __name__ == "__main__":
    om_image_split("E8_G_XZ.png", SAVE_TEST_PATH, PX_SQUARE_IMAGE_SIZE)
    print("done")
